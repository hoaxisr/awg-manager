...
### Исправлено
- **Восстановление мёртвых туннелей** — если мониторинг обнаруживал потерю связи и перезапускал туннель, часть настроек (маршруты, firewall) могла не применяться. Теперь перезапуск работает так же надёжно, как ручное выключение и включение
- **Счётчик ошибок в логах мониторинга** — счётчик мог показывать «FAIL 0/3» или скакать назад, из-за чего казалось, что он сломан. Исправлено
- **Неправильный порт в адресе после установки** — если порт 2222 был занят, программа выбирала другой порт, но в сообщении при установке и в статусе всё равно показывала `http://...:2222`. Теперь всегда отображается правильный адрес
- **Ошибка обновления на некоторых роутерах** — обновление могло не работать из-за отсутствия `wget-ssl`. Теперь устанавливается автоматически при необходимости
- **Маршрут по умолчанию на kernel-бэкенде** — на некоторых конфигурациях туннель на kernel-модуле не получал default route

### Добавлено
- **Страница диагностики** — три режима: быстрый тест, полная проверка, отчёт для поддержки. Результаты отображаются в реальном времени
- **Таймер восстановления** — для мёртвых туннелей показывается обратный отсчёт «Восстановление через 1:45» до следующей попытки перезапуска
- **Подтверждение смены бэкенда** — при переключении с userspace на kernel показывается предупреждение о возможной потере связи с роутером
- **Обновление без разрыва соединений** — при обновлении программы туннели больше не перезапускаются, демон подключается к уже работающим процессам
- Поддержка Keenetic Ultra II (kernel-модуль)
- Иконка для добавления на главный экран (iOS/Android)
