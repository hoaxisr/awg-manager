...
describe('menuMemory', () => {
	...
	// Схема ключа общая с уже сохранёнными данными пользователей и объявлена
	// неизменяемой: смена разделителя или порядка сортировки обесценит память.
	// Остальные тесты такую смену переживут молча, этот — нет.
	it('литерал ключа зафиксирован', () => {
		expect(menuMemoryKey('Sing-box', ['b', 'a'])).toBe('ui.tabs.menuLast:Sing-box:a,b');
	});
