<script lang="ts" module>
	export interface ServerItem {
		...
		peerCount?: number;
		kind: 'managed' | 'system';
		/** Подпись бейджа. Системные бывают двух видов — встроенный VPN-сервер и
		    перенесённый интерфейс, — и карточка их различает. */
		badge: string;
	}
</script>
...
<nav class="strip" aria-label="Список серверов">
	{#each items as item (item.id)}
		...
		>
			...
			<span class="badge badge-{item.kind}">{item.badge}</span>
		</button>
	{/each}
...
<style>
	/* Sticky: старый рейл был `position: sticky` и оставался на экране при
	   ...
	   поля, чтобы фон доходил до краёв контейнера. Боковые поля считаем от
	   --layout-gutter-x, а не числом: гуттер разный в обычном режиме (1.5rem),
	   на мобильном (0.75rem) и в компактном (0), и захардкоженное значение
	   давало то вылет за край, то щель между фоном полосы и краем колонки. */
	.strip {
		position: sticky;
		top: 0;
		...
		flex-wrap: wrap;
		gap: 0.625rem;
		padding: 0.75rem var(--layout-gutter-x);
		margin: -0.75rem calc(-1 * var(--layout-gutter-x)) 0.25rem;
	...
	@media (max-width: 640px) {
		/* Sticky снимаем: стопка плашек во всю ширину занимает половину экрана
		   и, залипнув, не давала листать детальную карточку. Старый рейл на
		   мобильном sticky тоже не был — прятался за дропдауном в потоке.
		   Боковые поля отдельно не задаём: они уже считаются от гуттера. */
		.strip {
			position: static;
		}
