<script lang="ts">
    import type { Policy, TunnelListItem, HotspotClient, DnsRouteTunnelInfo } from '$lib/types';
    ...
    let { open, tunnels, systemTunnels = [], hotspotClients, oncreate, onclose, saving }: Props = $props();
...
<Modal {open} title="Новая политика доступа" onclose={onclose}>
        ...
        <select class="form-select" id="policy-tunnel" bind:value={tunnelID}>
            ...
            {#if systemTunnels.length > 0}
                ...
                {#each systemTunnels as tunnel}
                    <option value={tunnel.id}>{tunnel.name} (системный)</option>
                {/each}
            {/if}
        </select>
    </div>
    ...
    {#snippet actions()}
        ...
        <button class="btn btn-primary" onclick={handleCreate} disabled={!canCreate || saving}>
