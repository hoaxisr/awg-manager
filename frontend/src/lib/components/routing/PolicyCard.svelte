<script lang="ts">
    import type { Policy, TunnelListItem } from '$lib/types';
    ...
    let { policies, tunnels, ontoggle, ondelete, oncreate, saving }: Props = $props();

    function tunnelName(tunnelID: string): string {
        const t = tunnels.find(t => t.id === tunnelID);
        return t ? t.name : tunnelID;
    }

    function isOrphaned(tunnelID: string): boolean {
        return !tunnels.some(t => t.id === tunnelID);
    }
</script>

<div>
    <div class="section-label">Политика доступа</div>
    <div class="card">
        {#if policies.length === 0}
            <p class="section-hint">Маршрутизация трафика LAN-клиентов через туннели.</p>
            <button class="btn btn-sm btn-accent" onclick={oncreate}>Добавить</button>
        {:else}
