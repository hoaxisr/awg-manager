<script lang="ts">
	...
	async function copyToClipboard() {
		...
		const text = filteredLogs.map(log => {
			...
			const result = log.stateChange === 'dead' ? 'DEAD' : log.stateChange === 'alive' ? 'RECOVERED' : log.stateChange === 'forced_restart' ? (log.success ? 'RESTART OK' : 'RESTART FAIL') : log.success ? 'OK' : 'FAIL';
			...
			<table>
				...
				<tbody>
					{#each visibleLogs as log}
						<tr class:log-error={!log.success} class:log-state-change={log.stateChange}>
