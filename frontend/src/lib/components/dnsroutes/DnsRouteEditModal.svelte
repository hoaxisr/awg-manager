<script lang="ts">
	...
	let availableTunnels = $derived(tunnels.filter((t) => !routes.some((r) => r.tunnelId === t.id)));
	...
	function addRoute() {
		...
		const cleared = routes.map((r) => ({ ...r, fallback: '' }));
	...
	<div class="form-section">
		...
		{#if availableTunnels.length > 0}
			<div class="route-add">
				...
				<button class="btn btn-sm btn-accent" onclick={addRoute}>+ Добавить</button>
			</div>
		{/if}
