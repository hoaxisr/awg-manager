...
describe('singboxRuleToCard', () => {
  ...
  // Ревью нашло: бэкенд отвергает managed-правило в bulk-outbound, а
  // «Выбрать все» его захватывал — и весь батч падал с 400.
  it('marks rules generated by awg-manager so selection can skip them', () => {
    const managed = singboxRuleToCard(
      { ip_cidr: ['1.2.3.4/32'], outbound: 'vpn', awgm_managed: 'selective-ip' },
      0, [], {},
    );
    expect(managed.isManaged).toBe(true);
    expect(managed.isSystem).toBe(false);

    const own = singboxRuleToCard({ ip_cidr: ['1.2.3.4/32'], outbound: 'vpn' }, 0, [], {});
    expect(own.isManaged).toBe(false);
  });
