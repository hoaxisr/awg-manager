...
<script lang="ts">
  ...
  // Гарантия «весь TCP/UDP членов политики идёт через sing-box» неверна при
  // селективном обходе: с ipset в движок попадает только трафик к адресам из
  // правил — ровно то, что говорит секция «Селективный перехват».
  let awgmGuaranteeHolds = $derived(tproxyMode && !(cfg?.selectiveBypass ?? false));
    ...
    {#if isExpert && cfg}
      ...
      <section class="sec">
        ...
        {:else}
          ...
          <p class="hint">
            {#if awgmGuaranteeHolds}Пока режим включён, весь TCP- и UDP-трафик членов политики
