...
<script lang="ts">
  ...
  // при старом маршруте. Спрашиваем persisted-режим, а не targetMode: тот сводит
  // fakeip к 'tproxy'. Пустой режим — легаси-tproxy, как и в бэкенде.
  let tproxyMode = $derived(!cfg?.routingMode || cfg.routingMode === 'tproxy');
