...
<script lang="ts">
  ...
  // Режим, который поднимется при включении: активный, иначе выбранный в этой
  // сессии, иначе persisted (пустой — легаси-tproxy, как и в бэкенде). Отдельно
  // от targetMode: тот типизирован двумя режимами дровера и схлопывает
  // fakeip-tun в 'tproxy', а гейт ниже обязан отличать именно его. Сузить
  // targetMode нельзя — на нём висят соседние деривации.
  let runMode = $derived(activeMode ?? pickedMode ?? (cfg?.routingMode || 'tproxy'));
  // Заглушка на время подъёма и catch-all перехват живут ТОЛЬКО в tproxy-ветке
  // провизии: fakeip-tun и policy-tun возвращаются раньше неё и ждут готовности
  // при старом маршруте. Спрашиваем «что включится», а не persisted-режим:
  // выбор при выключенном движке в настройки не пишется.
  let tproxyMode = $derived(runMode === 'tproxy');
  // Гарантия «весь TCP/UDP выбранного источника идёт через sing-box» неверна при
  // селективном обходе: с ipset в движок попадает только трафик к адресам из
  // правил — ровно то, что говорит секция «Селективный перехват». И неверна без
  // conntrack: вторая её половина — вытеснение потоков, ушедших мимо перехвата,
  // а без бинаря скрипт молча выходит нулём (об этом же говорит предупреждение
  // ниже — без этого условия оба текста стояли бы на экране одновременно и
  // противоречили друг другу).
  //
  // Именно `!== false`, а не `=== true`: на бэкенде без этого поля приезжает
  // undefined, и гарантия не должна пропадать из-за отсутствия признака.
  let awgmGuaranteeHolds = $derived(
    tproxyMode && !(cfg?.selectiveBypass ?? false) && s?.conntrackAvailable !== false,
  );
      ...
      <section class="sec">
        ...
        {:else}
          <p class="hint warn-text">
            ...
            они переустановятся сами, уже через прокси. {#if tproxyMode}Пока движок поднимается,
            трафик членов политики заблокирован — до минуты.{/if}
          </p>
          <p class="hint">
            <!-- «выбранного источника», а не «членов политики»: в режиме «все
                 устройства» политики нет вовсе, спека идёт с MatchAll. Состав
                 источника задаётся секцией «Источник трафика». -->
            {#if awgmGuaranteeHolds}Пока режим включён, весь TCP- и UDP-трафик выбранного источника
            идёт через sing-box: потоки, успевшие уйти мимо перехвата, вытесняются.{/if}
        ...
        {/if}
        <!-- Путь продублирован из Go-константы conntrackBinPath: по сети едет
             булево, а не путь, автоматической сверки нет — менять оба места. -->
