...
//
// ЕДИНСТВЕННЫЙ ПОТРЕБИТЕЛЬ — StatusDrawer, которую не монтирует ни один
// маршрут. Страница «Движок» на эту идиому НЕ опирается: тумблер выражал
// невалидную половину пары (wanAutoDetect, wanInterface), и его выключение не
// сохранялось вообще, оставляя интерфейс выключенным только на вид. Там теперь
// один список (sb-engine/wanSelect.ts) — новых потребителей заводить здесь не
// нужно.
