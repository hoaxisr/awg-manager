<script lang="ts">
	...
	// Only track status, isDeadByMonitoring, and lastHandshake to avoid
	// re-running on every poll update of the tunnel object
	$effect(() => {
		const status = tunnel?.status;
		const isDead = tunnel?.isDeadByMonitoring;
