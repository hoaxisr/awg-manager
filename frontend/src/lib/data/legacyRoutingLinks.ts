...
/**
 * Все страницы, куда способен увести редирект — без повторов.
 *
 * Экспорт нужен сверке в `routes/sb/engineRoutes.test.ts`: таблицы выше — это
 * четвёртый список маршрутов группы, и переименование страницы волной 5D2 иначе
 * уронит три остальных списка, а редирект молча уведёт закладки в 404.
 */
export const REDIRECT_TARGETS: readonly string[] = [
	...new Set([...SUB_TARGETS.values(), ...CHIP_TARGETS.values()]),
];
