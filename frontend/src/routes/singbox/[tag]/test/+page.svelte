<script lang="ts">
	...
	import type { IPResult, ConnectivityResult, IPCheckService, SingboxTunnel, SpeedTestInfo, SpeedTestResult } from '$lib/types';
	...
	async function checkConnectivity() {
		...
		try {
			connectivityResult = await api.singboxCheckConnectivity(tunnelTag);
		} catch (e) {
	...
	async function checkIP() {
		...
		ipResult = null;
		try {
			ipResult = await api.singboxCheckIP(tunnelTag, serviceURL || undefined);
		} catch (e) {
	...
	function friendlyError(msg: string): string {
		...
		if (msg.includes('no kernel interface')) {
			return 'Интерфейс туннеля недоступен';
		}
		return msg;
	}
	...
	function runStream(server: string, port: number): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			activeEventSource = api.singboxSpeedTestStream(
				...
				tunnel?.kernelInterface ?? undefined,
			);
		});
	}
...
<div class="page-header test-page-header">
	<a href="/?tab=singbox" class="back-link">
...
<div class="tests-grid">
	{#if !tunnelLoaded}
		<div class="card test-card">
			<h3>Загрузка...</h3>
			<p class="test-desc">Проверяем доступность sing-box туннеля.</p>
		</div>
	{:else if tunnel === null}
		<div class="card test-card">
			<h3>Тестирование недоступно</h3>
			<p class="test-desc">Туннель не найден или недоступен для расширенного тестирования.</p>
		</div>
	{:else if !tunnel.kernelInterface}
		<div class="card test-card">
			<h3>Тестирование недоступно</h3>
			<p class="test-desc">У этого sing-box туннеля нет kernel interface, расширенные тесты недоступны.</p>
		</div>
	{:else}
	<div class="card test-card">
		<h3>
