<script lang="ts">
	...
	import { type ProtocolKey, protocols, getSignaturePackets } from '$lib/utils/protocols';
	...
	let selectedProtocol = $state<ProtocolKey>('quic');

	function generateSignaturePackets() {
		const packets = getSignaturePackets(selectedProtocol);
		$form.i1 = packets.i1;
		$form.i2 = packets.i2;
		$form.i3 = packets.i3;
		$form.i4 = packets.i4;
		$form.i5 = packets.i5;
		notifications.success(`Сгенерированы сигнатуры для ${protocols[selectedProtocol].name}`);
	}
	...
	<div class="edit-wrapper">
		...
		<div class="content-layout">
			...
			<form class="left-column" onsubmit={(e) => { e.preventDefault(); handleSaveAndStart(); }}>
				...
				<section class="form-section">
					<h2 class="section-title">Маскировка трафика</h2>
					<p class="section-desc">
						Сигнатурные пакеты (I1–I5) определяют первые байты соединения.
						DPI-системы анализируют эти байты для классификации трафика.
						Выберите протокол, чтобы трафик выглядел как легитимный.
					</p>
					...
					<div class="generate-row">
						...
						<button type="button" class="btn-generate" onclick={generateSignaturePackets}>
					...
					<div class="cps-hints">
						<div class="cps-header">
							...
							<span>Теги CPS для ручного редактирования</span>
						</div>
						<div class="cps-grid">
							<div class="cps-row"><code>&lt;r X&gt;</code> <span>X случайных байт — уникальность каждого пакета</span></div>
							<div class="cps-row"><code>&lt;c&gt;</code> <span>Счётчик — увеличивается с каждым пакетом</span></div>
							<div class="cps-row"><code>&lt;t&gt;</code> <span>Timestamp UNIX — текущее время</span></div>
							<div class="cps-row"><code>&lt;b 0xXX&gt;</code> <span>Статические байты — фиксированная сигнатура протокола</span></div>
						</div>
					</div>
				</section>
			...
			<aside class="right-column">
				<div class="card">
					...
					<AWGAdvancedParams
						...
						onGenerate={generateSignaturePackets}
