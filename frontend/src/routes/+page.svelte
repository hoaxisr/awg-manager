<script lang="ts">
	...
	import { TunnelCard, ExternalTunnelCard, AdoptTunnelDialog } from '$lib/components/tunnels';
	...
	let showUnsupportedBlock = $derived(
		systemInfo !== null &&
		systemInfo.kernelModuleDownloadStatus === 'unsupported' &&
		!systemInfo.kernelModuleLoaded &&
		systemInfo.activeBackend !== 'userspace'
	);
	...
	function readAndImport(file: File) {
		...
		reader.onload = async (e) => {
			...
			try {
				...
				const tunnel = await tunnels.importConfig(content, name);
	...
	let statusLine = $derived.by(() => {
		...
		return `${systemInfo.version}  ·  ${systemInfo.goArch}  ·  0 туннелей`;
	});
...
<PageContainer>
	...
	{:else}
		<div class="tunnels-toolbar">
			<span class="tunnel-count">{$tunnels.length} {$tunnels.length === 1 ? 'туннель' : $tunnels.length < 5 ? 'туннеля' : 'туннелей'}</span>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			{#each $tunnels as tunnel (tunnel.id)}
				<TunnelCard
					{tunnel}
					activeBackend={systemInfo?.activeBackend}
