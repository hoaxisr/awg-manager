<script lang="ts">
	...
	onMount(() => {
		// Идемпотентно и на весь процесс. Это не только сокет: при запущенном
		// движке поднимается Clash-WS И бессрочный опрос /singbox/clients-by-ip
		// раз в 30 секунд (liveConnectionsStore.ts) — именами клиентов
		// подписываются соединения. При остановленном движке не открывается
		// ничего. Цена бейджа «Соединения движка», который по разделу 2 спеки
		// виден с любой страницы группы.
