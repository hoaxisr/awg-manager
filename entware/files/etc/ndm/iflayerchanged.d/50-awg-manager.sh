...
case "$id" in
    PPPoE*|ISP*|LTE*|WifiMaster*|WifiStation*|UsbModem*|UsbLte*|UsbQmi*|CdcEthernet*|PPTP*|L2TP*|IPoE*) ;;
    *) exit 0 ;;
esac
...
if [ "$layer" = "ipv4" ] && [ "$level" = "running" ]; then
    RESULT=$(/opt/bin/curl -s -o /dev/null -w '%{http_code}' \
        --max-time 5 -X POST "${BASE_URL}/api/wan/event?action=up&interface=${id}" 2>&1)
    exit 0
fi
...
if [ "$layer" = "ipv4" ] && [ "$level" = "disabled" ]; then
    RESULT=$(/opt/bin/curl -s -o /dev/null -w '%{http_code}' \
        --max-time 3 -X POST "${BASE_URL}/api/wan/event?action=down&interface=${id}" 2>&1)
    exit 0
fi
