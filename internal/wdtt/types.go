...
type ServerClient struct {
	...
	// опоры на пути старта (syncServerClientsOnStart → ensureUsableServerClient).
	// На путях UI абонент за пользователя не заводится (Дополнение №5), опора
	// сохранения конфига снята. Признак ХРАНИТСЯ, потому что вычислить его
	// нечем: имя пользователь меняет, и матч по "Абонент 1" соврал бы сразу
...
type ProcessStatus struct {
	...
	// обычным стартом — process.Start его усыновляет. Сам собой этот старт
	// случается только у КЛИЕНТА: серверный цикл супервизора смотрит на
	// IsRunning (supervisor.go), осиротевший сервер для него запущен, и признак
	// висит до ручного перезапуска. Не путать со stalePid у HydraRoute: там
