...
// ── сброс паузы анти-флаппинга ───────────────────────────────────

// resetSpy помнит КЛЮЧИ, с которыми звали сброс, и пароль записи на момент
// вызова: сброс не тому инстансу либо сброс ДО правки записи прошли бы
// незамеченными при сверке одного лишь факта вызова.
...
func (r *resetSpy) reset(key string) {
	r.keys = append(r.keys, key)
	pass := ""
	if rec, ok := r.src.Get(key); ok && rec.WdttClient != nil {
		pass = rec.WdttClient.Password
	}
	r.passwords = append(r.passwords, pass)
}

func newServiceReset(t *testing.T, rec instancestore.Record, f *fakeFetch) (*Service, *fakeMutator, *resetSpy) {
	t.Helper()
	src := &fakeSource{recs: map[string]instancestore.Record{rec.Key(): rec}}
	mut := &fakeMutator{src: src}
	spy := &resetSpy{src: src}
	return New(Deps{Records: src, Mutator: mut, Fetch: f.fetch,
		ResetStartBackoff: spy.reset}), mut, spy
}

func newFetchWithNewPassword() *fakeFetch {
	return &fakeFetch{res: subscriptionOf(
		wdttlink.ImportPayload{Name: "Германия-2", Peer: "1.2.3.4:56000", Password: "новый"},
	)}
}

// Клиент, который не поднимается, набирает паузу повторного старта до пяти
// минут. Обновление подписки — явное действие человека, который его чинит:
// прежние неудачи больше не показательны, и ждать паузу нельзя.
func TestRefresh_ResetsStartBackoffAfterUpdate(t *testing.T) {
	s, mut, spy := newServiceReset(t, clientRecord(), newFetchWithNewPassword())

	if _, err := s.Refresh(context.Background(), clientKey); err != nil {
		t.Fatalf("refresh: %v", err)
	}
	if !reflect.DeepEqual(spy.keys, []string{clientKey}) {
		t.Fatalf("сброс позван с %#v, ожидали ровно один вызов с %q", spy.keys, clientKey)
	}
	// Пароль на момент сброса — уже новый: сброс ДО записи снял бы паузу, а
	// стартовал бы процесс со старым профилем и снова упал.
	if !reflect.DeepEqual(spy.passwords, []string{"новый"}) {
		t.Fatalf("на момент сброса пароль %#v, ожидали новый — сброс идёт ПОСЛЕ правки записи", spy.passwords)
	}
	if len(mut.updates) != 1 {
		t.Fatalf("правок записи %d, ожидали одну", len(mut.updates))
	}
}

// Пауза снимается только на состоявшееся обновление: неудача — это ровно тот
// случай, ради которого анти-флаппинг существует.
func TestRefresh_NoResetWithoutUpdate(t *testing.T) {
	t.Run("подписка не загрузилась", func(t *testing.T) {
		f := &fakeFetch{err: errors.New("таймаут")}
		s, _, spy := newServiceReset(t, clientRecord(), f)
		if _, err := s.Refresh(context.Background(), clientKey); err == nil {
			t.Fatal("отказ загрузки обязан вернуть ошибку")
		}
		if len(spy.keys) != 0 {
			t.Fatalf("сброс на неудаче: %#v", spy.keys)
		}
	})

	t.Run("запись не сохранилась", func(t *testing.T) {
		s, mut, spy := newServiceReset(t, clientRecord(), newFetchWithNewPassword())
		mut.fail = errors.New("хранилище недоступно")
		if _, err := s.Refresh(context.Background(), clientKey); err == nil {
			t.Fatal("отказ мутатора обязан вернуть ошибку")
		}
		if len(spy.keys) != 0 {
			t.Fatalf("сброс при непринятой правке: %#v", spy.keys)
		}
	})
