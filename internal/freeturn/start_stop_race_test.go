...
// его снова. Зеркало internal/wdtt/start_stop_race_test.go.
//
...
func newRaceService(t *testing.T) *Service {
	t.Helper()
	dir := t.TempDir()
	return NewService(dir, dir, filepath.Join(dir, "ft-client"), filepath.Join(dir, "ft-server"))
}
...
// Второй одновременный старт сервера отступает без работы — супервизор не
// должен стартовать инстанс поверх старта, идущего из API.
