...
func TestStartClient_ListenRepairSyncsLinkedEndpoints(t *testing.T) {
	...
	s.SetLinkedEndpointSync(func(clientID, listen string) (int, error) {
		calls = append(calls, syncCall{clientID, listen})
		return 1, nil
	})
	...
	if len(calls) != 1 {
		t.Fatalf("ожидался один вызов sync, получено %d: %+v", len(calls), calls)
	}
	if calls[0].clientID != DefaultInstanceID {
		t.Errorf("clientID = %q, ожидался %q", calls[0].clientID, DefaultInstanceID)
	}
	if calls[0].listen == "127.0.0.1:9000" {
		t.Errorf("sync получил старый listen %q — endpoint останется протухшим", calls[0].listen)
	}
	got, err := s.GetConfig()
	if err != nil {
		t.Fatal(err)
	}
	if calls[0].listen != got.Clients[0].Config.Listen {
		t.Errorf("sync получил listen %q, в конфиге %q", calls[0].listen, got.Clients[0].Config.Listen)
	}
}
