...
func TestParsePoliciesRC_StandaloneShapes(t *testing.T) {
	cases := map[string]struct {
		body string
		want bool
	}{
		"boolean true":   {body: `"standalone": true`, want: true},
		"empty object":   {body: `"standalone": {}`, want: true},
		"boolean false":  {body: `"standalone": false`, want: false},
		"null":           {body: `"standalone": null`, want: false},
		"key absent":     {body: `"description": "X"`, want: false},
	}
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			raw := []byte(`{"Policy0": {` + tc.body + `}}`)
			got, err := parsePoliciesRC(raw)
			if err != nil {
				t.Fatal(err)
			}
			if got["Policy0"].standalone != tc.want {
				t.Errorf("standalone = %v, want %v", got["Policy0"].standalone, tc.want)
			}
		})
	}
}
