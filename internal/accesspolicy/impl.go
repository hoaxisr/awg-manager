...
func (s *ServiceImpl) lookupHostIP(ctx context.Context, mac string) string {
	...
	for _, h := range hosts {
		if strings.ToLower(h.MAC) == want && h.IP != "" {
			return h.IP
		}
	}
	// Дизайн исходил из того, что адрес известен всегда, — это не так: записи
	// нет у устройства за нижестоящим роутером, а запись без аренды бывает у
	// статического хоста и у только что выключенного устройства. Молчать здесь
	// нельзя: вытеснение не состоялось, гарантия на этом устройстве не
	// сработала, и без строки в журнале это неотличимо от успешного прохода.
	s.appLog.Info("evict-flows", mac,
		"адрес устройства не найден в хотспоте — потоки, ушедшие мимо перехвата, не вытеснены")
	return ""
}
