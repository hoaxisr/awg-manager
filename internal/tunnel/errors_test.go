...
func TestWrapOpError(t *testing.T) {
	// Test wrapping nil
	if got := WrapOpError("start", "awg0", nil); got != nil {
		t.Errorf("WrapOpError(nil) = %v, want nil", got)
	}

	// Test wrapping regular error
	regularErr := errors.New("some error")
	wrapped := WrapOpError("start", "awg0", regularErr)
	var opErr *OpError
	if !errors.As(wrapped, &opErr) {
		t.Error("WrapOpError should return *OpError")
	}

	// Test wrapping OpError (should not double-wrap)
	alreadyWrapped := &OpError{Op: "stop", TunnelID: "awg1", Err: regularErr}
	doubleWrapped := WrapOpError("start", "awg0", alreadyWrapped)
	if doubleWrapped != alreadyWrapped {
		t.Error("WrapOpError should not double-wrap OpError")
	}
}
