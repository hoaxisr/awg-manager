...
// TestStateMatrixV2_Scenario_PingCheckDead_Userspace verifies state after
// PingCheck kills the userspace process.
// NDMS: state: up, conf: running (intent preserved), link: down
func TestStateMatrixV2_Scenario_PingCheckDead_Userspace(t *testing.T) {
	m := StateMatrixV2{}

	input := StateInputs{
		HasNDMS:        true,
		OpkgTunExists:  true,
		Intent:         ndms.IntentUp, // conf: running (preserved after kill)
		LinkUp:         false,
		ProcessRunning: false, // killed by PingCheck
	}

	got := m.DetermineState(input)
	if got != tunnel.StateNeedsStart {
		t.Errorf("PingCheck dead (userspace): got %v, want StateNeedsStart", got)
	}
}
