...
// AppLogger logs events visible in the web UI log page.
...
type Operator interface {
	...
	SetupEndpointRoute(ctx context.Context, tunnelID, endpoint, ispInterface string) (string, error)
	...
	// Policy-based routing (OS4: ip rule + routing tables, OS5: no-op placeholder)

	// SetupPolicyTable creates a routing table entry for a tunnel with default + LAN routes.
	SetupPolicyTable(ctx context.Context, tunnelIface string, tableNum int) error

	// CleanupPolicyTable flushes all routes from a routing table.
	CleanupPolicyTable(ctx context.Context, tableNum int) error
	...
	// SetQlen sets txqueuelen on a running tunnel interface.
	SetQlen(ctx context.Context, tunnelID string, qlen int) error
	...
	SetAppLogger(logger AppLogger)
}

// OperatorOS5 is the Operator implementation for Keenetic OS 5.0+.
// Uses NDMS for interface management.
type OperatorOS5 struct{}

// OperatorOS4 is the Operator implementation for Keenetic OS 4.x.
// Uses ip commands directly.
type OperatorOS4 struct{}
