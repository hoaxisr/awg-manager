...
// Performs full cross-backend cleanup to handle backend mode switches.
func (o *OperatorOS4Impl) Recover(ctx context.Context, tunnelID string, state tunnel.StateInfo) error {
	...
	// Stop via current backend
	_ = o.backend.Stop(ctx, ifaceName)

	// Kill stale userspace process (handles kernel←userspace switch)
	if o.backend.Type() == backend.TypeKernel {
...
func (o *OperatorOS4Impl) SetupEndpointRoute(ctx context.Context, tunnelID, endpoint, _ string) (string, error) {
	return "", nil
}
...
func (o *OperatorOS4Impl) SetQlen(ctx context.Context, tunnelID string, qlen int) error {
	if o.backend.Type() != backend.TypeKernel {
	...
	if _, err := exec.Run(ctx, "/opt/sbin/ip", "link", "set", "dev", tunnelID, "txqueuelen", fmt.Sprintf("%d", qlen)); err != nil {
