...
type MockNDMSClient struct {
	...
	AddHostRouteCalls    []struct{ Host, Via string }
...
func (m *MockNDMSClient) IsInterfaceUp(ctx context.Context, name string) bool {
...
func (m *MockNDMSClient) AddHostRoute(ctx context.Context, host, via string) error {
	m.AddHostRouteCalls = append(m.AddHostRouteCalls, struct{ Host, Via string }{host, via})
	return nil
}
...
func (m *MockNDMSClient) GetDefaultIPv6Gateway(ctx context.Context) (string, string, error) {
	return m.GetDefaultGateway(ctx)
}

func (m *MockNDMSClient) GetDefaultIPv6GatewayWithRetry(ctx context.Context) (string, string, error) {
	return m.GetDefaultIPv6Gateway(ctx)
}

func (m *MockNDMSClient) GetIPv6GatewayForInterface(ctx context.Context, iface string) (string, error) {
	return m.GetGatewayForInterface(ctx, iface)
}
...
func (m *MockNDMSClient) GetDefaultGatewayInterfaceWithRetry(ctx context.Context) (string, error) {
	return m.GetDefaultGatewayInterface(ctx)
}

func (m *MockNDMSClient) GetDefaultGateway(ctx context.Context) (string, string, error) {
	if m.gatewayError != nil {
		return "", "", m.gatewayError
	}
	gw := m.defaultGatewayIP
	if gw == "" {
		gw = "10.0.0.1"
	}
	iface := m.defaultGateway
	if iface == "" {
		iface = "PPPoE1"
	}
	return gw, iface, nil
...
func (m *MockNDMSClient) GetDefaultGatewayWithRetry(ctx context.Context) (string, string, error) {
	return m.GetDefaultGateway(ctx)
}

func (m *MockNDMSClient) GetGatewayForInterface(ctx context.Context, iface string) (string, error) {
	if m.gatewayError != nil {
		return "", m.gatewayError
	}
	gw := m.defaultGatewayIP
...
func (m *MockNDMSClient) IsDHCPClientBound(ctx context.Context, iface string) bool {
	return false
}
...
func TestOperatorOS5_Start_Success(t *testing.T) {
	...
	// When OpkgTun exists, NDMS SetAddress/SetMTU are skipped (justCreated=false)
	...
	if len(ndms.SetMTUCalls) != 0 {
		t.Errorf("NDMS.SetMTU should NOT be called when OpkgTun exists, got %d", len(ndms.SetMTUCalls))
	}
	...
	// NDMS default route is set when DefaultRoute=true
	if len(ndms.SetRouteCalls) != 1 {
		t.Errorf("NDMS.SetDefaultRoute not called, got %d", len(ndms.SetRouteCalls))
	}
}
...
func TestOperatorOS5_Start_JustCreated_SetsNDMSConfig(t *testing.T) {
	...
	if ndms.SaveCalls != 1 {
		t.Errorf("NDMS.Save should be called, got %d", ndms.SaveCalls)
	}
}
...
func TestOperatorOS5_Start_BackendFails_Rollback(t *testing.T) {
	...
	// OpkgTun exists -> justCreated=false -> no SetAddress/SetMTU
	if len(ndms.SetAddrCalls) != 0 {
		t.Errorf("SetAddress should NOT be called when OpkgTun exists, got %d", len(ndms.SetAddrCalls))
	}
	if len(ndms.SetMTUCalls) != 0 {
		t.Errorf("SetMTU should NOT be called when OpkgTun exists, got %d", len(ndms.SetMTUCalls))
	}
}
...
func TestOperatorOS5_Stop_Success(t *testing.T) {
	...
	// RemoveDefaultRoute is NOT called in kernel stop (no-op for kernel)
}
...
func TestOperatorOS5_SetupEndpointRoute_Success(t *testing.T) {
	ndmsMock := &MockNDMSClient{defaultGatewayIP: "10.0.0.1", defaultGateway: "PPPoE1"}
	...
	ip, err := op.SetupEndpointRoute(context.Background(), "awg0", "1.2.3.4:51820", "")
	...
	// Kernel mode: ip route commands used (not NDMS AddHostRoute)
	if len(ndmsMock.AddHostRouteCalls) != 0 {
		t.Errorf("NDMS.AddHostRoute should NOT be called in kernel mode, got %d calls", len(ndmsMock.AddHostRouteCalls))
	}

	// Should have: (1) ip route del (stale cleanup), (2) ip route add
	foundRouteAdd := false
	for _, call := range recorder.Calls {
		if strings.Contains(call, "route add 1.2.3.4/32 via 10.0.0.1") {
			foundRouteAdd = true
		}
	}
	if !foundRouteAdd {
		t.Errorf("ip route add not called, got: %v", recorder.Calls)
	}
