...
// kernel config (MTU/qlen) or userspace MTU via NDMS → WG → ip link up → NDMS up → routes → firewall → save.
// NDMS address/MTU config is only applied when OpkgTun was just created (import flow).
// For userspace backend, MTU is re-applied via NDMS after TUN device creation (amneziawg-go ignores .conf MTU).
func (o *OperatorOS5Impl) Start(ctx context.Context, cfg tunnel.Config) error {
	...
	if o.backend.Type() == backend.TypeKernel {
		...
		// Userspace: amneziawg-go ignores MTU from .conf and uses default 1420.
		// Re-apply configured MTU via NDMS after the TUN device is created.
		if cfg.MTU > 0 {
			if err := o.ndms.SetMTU(ctx, names.NDMSName, cfg.MTU); err != nil {
				o.logWarn("start", cfg.ID, "Failed to set MTU via NDMS: "+err.Error())
			}
		}
	}
	...
	if cfg.Endpoint != "" {
		...
		if _, err := o.SetupEndpointRoute(ctx, cfg.ID, routeEndpoint, cfg.KernelDevice, cfg.ISPInterface); err != nil {
			...
			o.appLogWarn("start", cfg.ID, "Не удалось создать endpoint route: "+err.Error())
		} else {
	...
	// NDMS manages the route for both kernel and userspace backends.
	if cfg.DefaultRoute {
		if err := o.ndms.SetDefaultRoute(ctx, names.NDMSName); err != nil {
...
func (o *OperatorOS5Impl) Delete(ctx context.Context, tunnelID string) error {
	...
	// Userspace: TUN is removed by DeleteOpkgTun above, skip.
	if o.backend.Type() == backend.TypeKernel {
...
func (o *OperatorOS5Impl) Reconcile(ctx context.Context, cfg tunnel.Config) error {
	...
	if cfg.Endpoint != "" {
		...
		if _, err := o.SetupEndpointRoute(ctx, cfg.ID, routeEndpoint, cfg.KernelDevice, cfg.ISPInterface); err != nil {
			...
			o.appLogWarn("reconcile", cfg.ID, "Не удалось создать endpoint route: "+err.Error())
		} else {
			endpointRouteOK = true
		}
	} else {
...
func (o *OperatorOS5Impl) KillLink(ctx context.Context, tunnelID string) error {
	...
	if o.backend.Type() == backend.TypeKernel {
		...
		// Userspace mode: kill the process. TUN stays (owned by NDMS OpkgTun).
