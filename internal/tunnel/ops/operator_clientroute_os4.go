...
func (o *OperatorOS4Impl) SetupClientRouteTable(ctx context.Context, kernelIface string, tableNum int) error {
	tableStr := strconv.Itoa(tableNum)

	// Add default route through tunnel
	result, err := exec.Run(ctx, "/opt/sbin/ip", "route", "replace", "default", "dev", kernelIface, "table", tableStr)
	if err != nil {
		return fmt.Errorf("setup route table %d: default route: %w", tableNum, exec.FormatError(result, err))
	}

	// Detect LAN subnet via ip route show dev br0
	lanSubnet := o.detectLANSubnetOS4(ctx)
	if lanSubnet != "" {
		result, err := exec.Run(ctx, "/opt/sbin/ip", "route", "replace", lanSubnet, "dev", "br0", "table", tableStr)
		if err != nil {
...
func (o *OperatorOS4Impl) AddClientRule(ctx context.Context, clientIP string, tableNum int) error {
	...
	exec.Run(ctx, "/opt/sbin/ip", "rule", "del", "from", clientIP, "lookup", tableStr)

	// Add new rule
	result, err := exec.Run(ctx, "/opt/sbin/ip", "rule", "add", "from", clientIP, "lookup", tableStr, "priority", tableStr)
	if err != nil {
...
func (o *OperatorOS4Impl) RemoveClientRule(ctx context.Context, clientIP string, tableNum int) error {
	...
	result, err := exec.Run(ctx, "/opt/sbin/ip", "rule", "del", "from", clientIP, "lookup", tableStr)
	if err != nil {
...
func (o *OperatorOS4Impl) CleanupClientRouteTable(ctx context.Context, tableNum int) error {
	tableStr := strconv.Itoa(tableNum)

	// Flush all routes in the table
	result, err := exec.Run(ctx, "/opt/sbin/ip", "route", "flush", "table", tableStr)
	if err != nil {
	...
	for i := 0; i < 100; i++ {
		_, err := exec.Run(ctx, "/opt/sbin/ip", "rule", "del", "lookup", tableStr)
		if err != nil {
			break // No more rules
		}
	}
...
// detectLANSubnetOS4 detects the LAN subnet from br0 interface routes.
// Returns empty string if detection fails.
func (o *OperatorOS4Impl) detectLANSubnetOS4(ctx context.Context) string {
	result, err := exec.Run(ctx, "/opt/sbin/ip", "route", "show", "dev", "br0")
	if err != nil {
		o.logWarn("client-route", "br0", "Failed to detect LAN subnet: "+err.Error())
		return ""
	}
