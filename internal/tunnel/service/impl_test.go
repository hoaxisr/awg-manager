...
func (m *MockStateManager) GetStateLightweight(ctx context.Context, tunnelID string) tunnel.StateInfo {
	// In tests, lightweight returns the same as full state
	return m.GetState(ctx, tunnelID)
}
...
type MockOperator struct {
	...
	SetQlenCalls             []struct{ ID string; Qlen int }
...
func (m *MockOperator) SetupEndpointRoute(ctx context.Context, tunnelID, endpoint, ispInterface string) (string, error) {
...
func (m *MockOperator) SetQlen(ctx context.Context, tunnelID string, qlen int) error {
	m.SetQlenCalls = append(m.SetQlenCalls, struct{ ID string; Qlen int }{tunnelID, qlen})
	return nil
}
...
func (m *MockOperator) SetAppLogger(logger ops.AppLogger) {}
