...
func (s *ServiceImpl) Update(ctx context.Context, tunnelID string, cfg tunnel.Config) error {
	...
	if stateInfo.State == tunnel.StateRunning {
		...
		if cfg.Endpoint != "" && cfg.Endpoint != oldEndpoint {
			...
			} else if ip, err := s.operator.SetupEndpointRoute(ctx, tunnelID, stored.Peer.Endpoint, s.resolveKernelDevice(resolvedWAN), resolvedWAN); err != nil {
...
func (s *ServiceImpl) resolveWAN(ctx context.Context, ispInterface string) (string, error) {
	if ispInterface == "" {
		...
		ndmsID, err := s.operator.GetDefaultGatewayInterface(ctx)
		if err != nil {
		...
		return s.operator.GetSystemName(ctx, ndmsID), nil
	}
