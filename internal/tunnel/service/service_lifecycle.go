...
func (s *ServiceImpl) startInternal(ctx context.Context, tunnelID string) error {
	...
	if stored.ISPInterface != "" && !tunnel.IsTunnelRoute(stored.ISPInterface) && !s.wan.IsUp(resolvedWAN) {
	...
	// HasWANIPv6 uses NDMS RCI (works with NDMS interface names like "ISP", "PPPoE0").
	hasIPv6 := sysinfo.HasDefaultIPv6Route() && s.operator.HasWANIPv6(ctx, resolvedWAN)
	if !hasIPv6 {
