...
func TestValidateHeaderRanges(t *testing.T) {
	cases := []struct {
		...
		{"уникальные одиночные", [4]string{"10", "20", "30", "40"}, ""},
		{"дефолт WG 1-4 уникален и допустим модулем", [4]string{"1", "2", "3", "4"}, ""},
		{"все пустые = дефолт модуля 1-4, допустимо", [4]string{"", "", "", ""}, ""},
		{"H1=2 при пустых остальных пересекается с дефолтом H2=2", [4]string{"2", "", "", ""}, "H1 и H2"},
		{"диапазон накрывает дефолт пустого H4=4", [4]string{"1-10", "20", "30", ""}, "H1 и H4"},
		{"равные одиночные — пересечение", [4]string{"10", "10", "30", "40"}, "H1 и H2"},
		{"диапазоны без пересечения", [4]string{"5-100", "101-200", "201-300", "301-400"}, ""},
		{"диапазон накрывает одиночное", [4]string{"5-100", "50", "201-300", "301-400"}, "H1 и H2"},
		{"пересечение через один", [4]string{"5-100", "200-300", "90-110", "400"}, "H1 и H3"},
		{"мусор — ошибка формата", [4]string{"abc", "2", "3", "4"}, "H1"},
	}
