...
// --- GenerateForStart tests ---

func TestGenerateForStart_WithIPv6(t *testing.T) {
	tunnel := &storage.AWGTunnel{
		Interface: storage.AWGInterface{
			PrivateKey: "privkey=",
		},
	...
	result := GenerateForStart(tunnel, true)
	assertContains(t, result, "::/0")
}

func TestGenerateForStart_WithoutIPv6(t *testing.T) {
	tunnel := &storage.AWGTunnel{
		Interface: storage.AWGInterface{
			PrivateKey: "privkey=",
		},
	...
	result := GenerateForStart(tunnel, false)
	assertNotContains(t, result, "::/0")
	assertContains(t, result, "0.0.0.0/0")
}

func TestGenerateForStart_WithoutIPv6_DoesNotMutateOriginal(t *testing.T) {
	tunnel := &storage.AWGTunnel{
		Interface: storage.AWGInterface{
			PrivateKey: "privkey=",
		},
	...
	GenerateForStart(tunnel, false)

	// Original tunnel should still have ::/0
	if len(tunnel.Peer.AllowedIPs) != 2 {
		t.Errorf("original AllowedIPs was mutated: %v", tunnel.Peer.AllowedIPs)
	}
}

// --- filterIPv6DefaultRoute tests ---

func TestFilterIPv6DefaultRoute_RemovesIPv6(t *testing.T) {
	result := filterIPv6DefaultRoute([]string{"0.0.0.0/0", "::/0"})
	if len(result) != 1 || result[0] != "0.0.0.0/0" {
		t.Errorf("result = %v, want [0.0.0.0/0]", result)
	}
}

func TestFilterIPv6DefaultRoute_KeepsOtherIPs(t *testing.T) {
	result := filterIPv6DefaultRoute([]string{"0.0.0.0/0", "::/0", "10.0.0.0/8"})
	if len(result) != 2 {
		t.Fatalf("result len = %d, want 2", len(result))
	}
}

func TestFilterIPv6DefaultRoute_Empty(t *testing.T) {
	result := filterIPv6DefaultRoute(nil)
	if len(result) != 1 || result[0] != "0.0.0.0/0" {
		t.Errorf("result = %v, want [0.0.0.0/0]", result)
	}
}

func TestFilterIPv6DefaultRoute_OnlyIPv6(t *testing.T) {
	result := filterIPv6DefaultRoute([]string{"::/0"})
	if len(result) != 1 || result[0] != "0.0.0.0/0" {
		t.Errorf("result = %v, want [0.0.0.0/0]", result)
	}
}
...
`
	...
	// DNS is a wg-quick field — silently dropped (by design)
	// Address is parsed
