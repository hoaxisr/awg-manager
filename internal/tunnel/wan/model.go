...
type Interface struct {
	Name     string // NDMS name: "PPPoE1", "ISP", "LTE0"
	Type     string // NDMS type: "PPPoE", "GigabitEthernet", "IPoE"
	Label    string // Human-readable label from NDMS description
...
// NonISPPrefixes are interface name prefixes that are NOT ISP connections
// despite having security-level: public (VPN tunnels, etc).
var nonISPPrefixes = []string{
	"Wireguard", "OpenVPN", "IPSec", "SSTP", "PPTP",
	"EoIP", "GRE", "IPIP",
}

// IsNonISPInterface returns true for interfaces that aren't ISP connections.
func IsNonISPInterface(name string) bool {
	for _, prefix := range nonISPPrefixes {
