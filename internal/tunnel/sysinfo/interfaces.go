...
// HasGlobalIPv6 checks whether the given network interface has a global-scope IPv6 address.
// Used to detect if the ISP provides IPv6 connectivity.
func HasGlobalIPv6(ifaceName string) bool {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		// Scope 00 = global, 20 = link-local, 80 = host
		if len(fields) >= 6 && fields[5] == ifaceName && fields[3] == "00" {
			return true
		}
	}
	return false
