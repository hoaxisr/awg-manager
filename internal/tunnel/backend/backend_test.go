...
func TestDetect(t *testing.T) {
	...
	if isKernelModuleAvailable() {
		if typ != TypeKernel {
			t.Errorf("Detect() = %v, want TypeKernel (module available)", typ)
		}
	} else {
		if typ != TypeUserspace {
			t.Errorf("Detect() = %v, want TypeUserspace (module not available)", typ)
		}
	}
}
...
func TestNew(t *testing.T) {
	b := New(TypeKernel)
	if b.Type() != TypeKernel {
		t.Error("New(TypeKernel) should return kernel backend")
	}

	b = New(TypeUserspace)
	if b.Type() != TypeUserspace {
		t.Error("New(TypeUserspace) should return userspace backend")
	}
}
...
func TestNewAuto(t *testing.T) {
	...
	if isKernelModuleAvailable() {
		if b.Type() != TypeKernel {
			t.Errorf("NewAuto() type = %v, want TypeKernel", b.Type())
		}
	} else {
		if b.Type() != TypeUserspace {
			t.Errorf("NewAuto() type = %v, want TypeUserspace", b.Type())
		}
	}
}
