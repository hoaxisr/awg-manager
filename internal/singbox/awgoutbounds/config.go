...
// fileShape is what 15-awg.json contains. Кроме outbounds слот
// объявляет dns.servers — по одному серверу на outbound, чтобы домен
// резолвился через сам туннель (#846). sing-box сливает config.d/*.json
// по ключам, поэтому inbounds/route здесь по-прежнему не объявляются.
...
// One direct outbound per entry, bound to its kernel iface. domain_resolver
...
// a valid JSON object — даже без записей файл содержит
