...
func (s *ServiceImpl) disablePolicyTun(ctx context.Context, settings *storage.Settings) error {
	...
	//
	// КРАШ-ОКНО между удержанием и записью персиста, и оно НЕ САМОЗАЖИВАЕТ.
	// Демон умер здесь — на диске Enabled=true, Provisioned=true, а интерфейс
	// положен, без адресов и без tun-инбаунда (его вырезал шаг 4). После
	// рестарта NDMS-объект жив, поэтому live[Index] истинен, полный
	// re-provision не запускается, а drift-heal возвращает только маршрут, слот
	// и permit — адрес, up и инбаунд не восстанавливает НИКТО. Режим числится
	// включённым, трафик членов политики дропается до ручного перевключения.
	//
	// До удержания окно закрывалось само: delete убивал интерфейс, live
	// становился ложным, следующий тик делал полный re-provision. Это цена
	// закреплённого индекса, а не недосмотр. Лечится ассертом в enabled-ветке
	// reconcile («провижинен и жив, но без адреса → переустановить»), которого
	// здесь сознательно нет: он выходит за рамки правки и требует отдельной
	// проверки, чтобы штатный рестарт sing-box не уходил в re-provision.
	//
