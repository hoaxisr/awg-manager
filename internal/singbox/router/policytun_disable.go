...
func (s *ServiceImpl) disablePolicyTun(ctx context.Context, settings *storage.Settings) error {
	...
	// осталось Provisioned=true при интерфейсе без адресов, и drift-heal вернёт
	// маршрут на положенный интерфейс, адрес не восстановив. Окно существовало и
	// до удержания (там же между delete и очисткой персиста), удержанный вариант
	// живёт в нём дольше — интерфейс не исчезает, поэтому heal считает его живым.
	//
