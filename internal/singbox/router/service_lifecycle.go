...
func (s *ServiceImpl) reconcileLocked(ctx context.Context) error {
	...
	case !sr.Enabled:
		...
		s.adoptOnce.Do(func() { _ = s.adoptAndClean(ctx) })
		// Заглушка АКТИВНОЙ раскладки — своей веткой: adopt выше снимает
		// blackhole только ЧУЖОГО канала. Загрузочная заглушка пишет свой файл
		// правил ДО restore, и смерть демона в окне подъёма (kill, питание,
		// паника) оставляет в настройках Enabled=false при живом DROP в ядре —
		// снять его больше некому, а в legacy-режиме DEAD-ветка netfilter.d-хука
		// вдобавок воскрешает заглушку по уцелевшему файлу на каждом
		// mangle/nat-событии. При выключенных настройках дропать policy-трафик
		// нельзя ни при каких обстоятельствах, поэтому вызов безусловный:
		// RemoveBlackhole идемпотентен (на чистой системе — пара отказавших
		// iptables) и заодно сносит файл правил, лишая хук источника. Ветку
		// «движок включён, но мёртв» это не задевает: там заглушка и есть
		// защита, и живёт она в reconcileInstalled.
		s.mu.Lock()
