...
func TestCtCleanScriptEvictsExplicitSourceIPs(t *testing.T) {
	...
	//
	// Всё ниже спрашивается у КОДА блока, без строк-комментариев: те же формы
	// (`-e src=`, `set -f`, имя токена) обсуждаются в комментариях рядом, и
	// сверка по сырому тексту давала бы то ложное срабатывание, то ложный
	// пропуск — этот класс ошибок в задаче уже стоил нескольких итераций.
	block := stripShellComments(awgmCtCleanBlock(t, body))
	...
	// Дальше — свойства, которые отбор не меняют и потому невидимы
	...
	// Базовый образец предфильтра — токен, а не сквозная подстрока вроде
	// `src=`, которая есть в КАЖДОЙ строке conntrack: со сквозной фильтр не
	// отсекает ничего и остаётся чистым накладным расходом на самом частом
	// пути. Отбор от этого не меняется — шапка awk при пустом evict и так
	// требует токен.
	if !strings.Contains(block, `set -- -F -e '[FASTNAT]'`) {
		t.Fatalf("предфильтр не отсекает по токену — на пустом списке он бесполезен:\n%s", block)
	}
	// Одного наличия строки выше мало: в дефектной редакции она тоже была, но
	// пряталась внутри `if [ -n "$evict_ips" ]`, а базовым образцом стоял
	// сквозной `-e src=`. Поэтому спрашиваем ещё и то, чего быть НЕ должно:
	// ветвления сборки образца по списку и сквозного образца как такового.
	// Адресный образец пишется как -e "src=$ip " — под запрет не подпадает.
	if strings.Contains(block, `-e src=`) {
		t.Fatalf("сквозной образец grep: подстрока src= есть в каждой строке conntrack, фильтр пропустит всё:\n%s", block)
	}
	if strings.Contains(block, `if [ -n "$evict_ips" ]`) {
		t.Fatalf("сборка образца снова ветвится по списку — пустая ветка опять станет сквозной:\n%s", block)
	}
	// Список адресов раскрывается без кавычек, то есть и с подстановкой имён
	// файлов: без set -f аргумент `*` развернулся бы по рабочему каталогу
	// демона, который никто не задаёт. Порядок обязателен: обе строки на
	// месте, но `set +f` над циклом — это снятая защита при целых уликах.
	fOn, loop, fOff := strings.Index(block, "set -f\n"), strings.Index(block, "for ip in $evict_ips"), strings.Index(block, "set +f\n")
	if fOn < 0 || loop < 0 || fOff < 0 || fOn > loop || loop > fOff {
		t.Fatalf("раскрытие списка адресов не обёрнуто в set -f … set +f (on=%d loop=%d off=%d):\n%s", fOn, loop, fOff, block)
	}
}
...
func TestCtCleanScriptSelectsExplicitSourceIPs(t *testing.T) {
	...
	// тут же — он под set -f не должен разворачиваться по каталогу.
...
// начнёт врать, поэтому равенство спрашивается явно.
//
// Присваивание спрашивается ЦЕЛОЙ строкой (якорь на \n с обеих сторон), а не
// подстрокой: тот же путь упомянут в пояснениях рядом, и подстрока была бы
// истинна от комментария — как от отдельной строки (её снимает
// stripShellComments), так и от ХВОСТОВОГО на строке чужого присваивания:
// `CT=/usr/sbin/conntrack # реальный путь: CT=/opt/sbin/conntrack`.
// Построчная очистка такой хвост не трогает, а ведущий \n — трогает: перед
// комментарием стоит пробел.
//
// Обратная сторона якоря: он требует ФОРМЫ — присваивание обязано стоять
// отдельной строкой, без отступа и не первым в теле скрипта (перед первой
// строкой перевода строки не будет). Переедет — тест упадёт на правке, которая
// поведения не меняет. Это намеренно и чинится переносом строки обратно, а не
// ослаблением предиката: цена ложной тревоги здесь много ниже цены пропуска —
// разъехавшийся путь означает, что статус говорит про один бинарь, а скрипт
// зовёт другой, и оба молчат.
