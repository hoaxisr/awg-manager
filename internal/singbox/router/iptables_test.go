...
func TestCtCleanScriptEvictsExplicitSourceIPs(t *testing.T) {
	...
	// `src=`, которая есть в каждой строке conntrack: со сквозной фильтр
	// вырождается в чистый накладной расход на самом частом пути (без
	// адресов), +15% на одном ядре. Отбор от этого не меняется — шапка awk
	// при пустом evict и так требует токен.
	...
	// демона, который никто не задаёт.
	if !strings.Contains(block, "set -f\n") || !strings.Contains(block, "set +f\n") {
		t.Fatalf("раскрытие списка адресов не защищено от подстановки имён файлов:\n%s", block)
	}
}
