...
if [ "$awgm_mode" -eq 1 ]; then
  ...
  # Предфильтр перед awk: разбор полей — самая дорогая часть прохода, и на
  # смене состава политики ему незачем видеть всю таблицу. grep -F дёшев и
  # отдаёт awk только строки нужных адресов ПЛЮС записи с токеном — вторые
  # нужны, чтобы скоуп прежнего прохода не сузился (иначе утечки прочих
  # устройств досидели бы до следующего вызова). Без адресов один -e src=
  # матчит любую строку conntrack, то есть фильтр вырождается в no-op.
