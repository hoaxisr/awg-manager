...
# Без путей здесь зовутся шесть: pidof, grep, sed, logger, uname, insmod.
# Худший случай — pidof: именно он решает ALIVE/DEAD. Ненайденный pidof даёт
# «движок мёртв» при живом движке, и хук выставляет fail-closed DROP, то есть
# рвёт трафик политики на ровном месте. Ненайденный grep не дешевле: на нём
# сидят и проверки джампа, и scrub_jumps. Проверки молча провалятся,
# mangle_ok/nat_ok останутся нулями, хук пойдёт лечить ОБЕ таблицы при целом
# перехвате, а scrub_jumps при этом ничего не удалит — и iptables-restore
# --noflush допишет второй джамп. На каждом срабатывании хука. Джампы копятся.
# Каталоги — те же, что в init-скрипте пакета, но дописаны в ХВОСТ: приоритет
# вызывающего не меняем.
export PATH="${PATH:+$PATH:}/bin:/sbin:/usr/bin:/usr/sbin:/opt/bin:/opt/sbin"
[ "$type" = "ip6tables" ] && exit 0
...
# Поэтому PATH задаём сами, а не полагаемся на вызывающего. Список каталогов и
# порядок — те же, что в init-скрипте пакета
# (entware/files/etc/init.d/S99awg-manager), но дописываем их в ХВОСТ, а не в
# голову, как он: приоритет вызывающего не трогаем — правка чинит нахождение
# бинаря, а не выбор между ними. ${PATH:+$PATH:} — чтобы при пустом PATH не приклеить ведущее
...
#
# Второй скоуп того же прохода — адреса из аргументов: у них сменилась
# принадлежность политике, и там предикат уязвимости неприменим (см. ниже).
#
...
if [ "$awgm_mode" -eq 1 ]; then
  [ -n "$evict_ips" ] && logger -t awgm-ctclean "policy membership changed for $evict_ips — their flows are in scope"
  fpm="$(printf '%%s\n' "$prerouting" \
    | sed -n 's/.*-m connmark --mark \(0x[0-9a-fA-F]*\).*-[jg] %[1]s.*/\1/p' | head -n 1)"
  [ -n "$fpm" ] && fpm="$(printf '%%d' "$fpm" 2>/dev/null)"
  # Предфильтр перед awk: разбор полей — самая дорогая часть прохода, и видеть
  # всю таблицу ему незачем ни на одном из путей. Токен в списке ВСЕГДА, без
  # ветвления по списку адресов: сборка образца вида -e src= была бы сквозной,
  # потому что подстрока src= есть в КАЖДОЙ строке conntrack — такой фильтр не
  # отсекает ни одной записи и остаётся чистым накладным расходом на самом
  # частом пути (аргументов нет). Отбор от токена в образце не меняется: шапка
  # awk при пустом evict и так требует /[FASTNAT]/. Адреса добавляются сверху:
  # их записи токена не несут, но нужны, и записи с токеном при этом остаются
  # в скоупе — иначе утечки прочих устройств досидели бы до следующего вызова.
  # Аргументы живут в позиционных параметрах, а не в переменной: хвостовой
  # пробел после адреса (якорь конца IP, иначе src=10.0.0.5 цепляет
  # src=10.0.0.55) не пережил бы разворачивание без кавычек, а мусор в адресе
  # — разбиение на слова. Ниже $@ всё равно перезаписывается списком WAN.
  # set -f на время цикла: это единственное место, где список раскрывается без
  # кавычек, а значит и с подстановкой имён файлов — '*' в аргументе ушёл бы
  # разворачиваться по рабочему каталогу демона, который никто не задаёт.
  set -f
  set -- -F -e '[FASTNAT]'
  ...
  set +f
  fastnat="$(grep "$@" /proc/net/nf_conntrack 2>/dev/null | awk -v pm="$fpm" -v evict="$evict_ips" '
    # Адреса, у которых сменилась принадлежность политике, — второй скоуп того
    # же прохода. split пустой строки даёт ноль элементов, поэтому без
    # аргументов множество пусто и отбор ниже вырождается в прежний.
    BEGIN { split(evict, a, " "); for (i in a) ev[a[i]] = 1 }
    ...
    # Пользовательских bypass-подсетей в списке нет, и отдельно они не нужны:
    # гард -j AWGMPPE эмитится без фильтра по назначению (emitPPEGuard), поэтому
    # установившийся поток к bypass-подсети несёт флаг и в выборку не идёт.
    # Вытеснится только тот, что родился в окне до постановки правил, — он и
    # переустановится сам, уже мимо прокси.
    ...
    (evict != "" || /\[FASTNAT\]/) && ($3 == "tcp" || $3 == "udp") {
      ...
      # Скоуп: поток, рождённый в LAN, а при заданной метке — ещё и с ней
      # (метка спрашивается ниже и только для адресов вне списка смены состава).
      # mac= требуется в ОБЕИХ ветках: он есть на каждом LAN-потоке Keenetic и
      # нет у собственных потоков роутера, а что метка политики никогда не
      # садится на поток роутера — не проверено. Цена ошибок несимметрична:
      # лишний пропуск оставляет утечку до конца потока, лишний разрыв рвёт
      # установившийся TCP роутера, вплоть до аплинка sing-box (issue #627).
      if (!hasmac) next;
      if (src == "" || dst == "" || sp == "" || dp == "") next;
      if (local_dst(dst)) next;
      # Адрес сменил принадлежность политике: у него протух ВЕСЬ conntrack, и
      # ни токен, ни метка тут не спрашиваются — вступившее устройство несёт
      # записи с меткой прежней политики (мимо скоупа по метке), вышедшее —
      # помеченные записи, которые иначе досидели бы в sing-box до конца
      # потока. Нужны обе категории, поэтому предикат уязвимости — только для
      # адресов ВНЕ списка.
      if (!(src in ev)) {
        if ($0 !~ /\[FASTNAT\]/) next;
        if (pm != "" && mark != pm) next;
