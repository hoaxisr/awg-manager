...
# рвёт трафик политики на ровном месте. Ненайденный grep дешевле, но тоже
# заметен: обе проверки джампа молча провалятся, mangle_ok/nat_ok останутся
# нулями, и хук уйдёт в полный restore вместо лечения одной таблицы.
...
if [ "$awgm_mode" -eq 1 ]; then
  ...
  # адресов шапка awk и так требует /[FASTNAT]/, поэтому отбор от этого не
  # меняется, а конвейер перестаёт гонять через себя все записи (сквозной
  # фильтр вроде -e src= стоил бы +15%% на одном ядре — целевые mips
  # одноядерные). Адреса добавляются сверху: их записи токена не несут, но
  # нужны, и записи с токеном при этом остаются в скоупе — иначе утечки
  # прочих устройств досидели бы до следующего вызова.
