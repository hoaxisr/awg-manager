...
var knownPresets = map[string]bypassPreset{
	...
	// адрес сервиса берётся с самого роутера (его статическая DNS-запись)
	// и приезжает в resolveBypassCIDRs отдельным аргументом, см.
	// SyncKeenDNSRewrites. Запись нужна, чтобы имя оставалось валидным
	// в BypassPresets.
	"keendns": {},
}
