...
func TestLogBuffer_GetByCategory(t *testing.T) {
	buf := NewLogBuffer()
	defer buf.Stop()

	// Add entries for different categories
	buf.Add(LogEntry{Timestamp: time.Now(), Category: CategoryTunnel, Message: "tunnel 1"})
	buf.Add(LogEntry{Timestamp: time.Now(), Category: CategorySettings, Message: "settings 1"})
	buf.Add(LogEntry{Timestamp: time.Now(), Category: CategoryTunnel, Message: "tunnel 2"})

	logs := buf.GetByCategory(CategoryTunnel)
	if len(logs) != 2 {
		t.Errorf("GetByCategory() len = %d, want 2", len(logs))
	}

	for _, log := range logs {
		if log.Category != CategoryTunnel {
			t.Errorf("Category = %s, want %s", log.Category, CategoryTunnel)
		}
	}
}
...
func TestLogBuffer_GetByLevel(t *testing.T) {
	...
	buf.Add(LogEntry{Timestamp: time.Now(), Level: LevelInfo, Message: "info"})
	buf.Add(LogEntry{Timestamp: time.Now(), Level: LevelError, Message: "error"})
	buf.Add(LogEntry{Timestamp: time.Now(), Level: LevelInfo, Message: "info 2"})

	logs := buf.GetByLevel(LevelError)
	if len(logs) != 1 {
		t.Errorf("GetByLevel() len = %d, want 1", len(logs))
	}
