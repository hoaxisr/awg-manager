...
func (l *Link) Close() {
	...
	c := l.cur
	l.cur = nil
	// Связь рвётся навсегда — рассказывать о процессе больше нечем.
	l.last = nil
	l.mu.Unlock()
	if c != nil {
		_ = c.Close()
...
// lost решает, что означает разрыв: вытеснение, смерть процесса или временную
// потерю связи.
//
// Вытеснение спрашивается У СОЕДИНЕНИЯ, а не ждётся событием. Событие evicted
// приезжает не всегда: сервер шлёт кадр и тут же закрывает сокет (awgmproto:
// send и Close в одной горутине), поэтому кадр либо теряется в переполненной
// очереди push (client.go: «очередь полна»), либо проигрывает разрыву в select
// самого watch — оба канала готовы, и выбор ветки случаен. Клиент же ставит
// свою защёлку ДО буферизации, и у соединения она есть всегда. Без этого
// вопроса вытеснение оборачивалось бы вердиктом «связь разорвана» →
// переподключение → взаимная пляска вытеснений двух менеджеров, ровно то,
// ради чего заведён ErrEvicted.
func (l *Link) lost(c *Client) {
	evicted := c.Evicted()
	l.mu.Lock()
	// evicted значим только на соединении, которое менеджер считает активным
	// (тот же довод, что в watch): брошенное соединение могло получить evicted
	// от переподключения этого же менеджера.
	if l.cur != c {
		evicted = false
	} else {
		l.cur = nil
	...
	if pid := l.knownPID(); pid > 0 && !l.alive(pid) {
		...
		l.forgetPID(pid)
		// Снимок описывал ВОПЛОЩЕНИЕ, которого больше нет. Оставить его —
		// значит вечно показывать «работает», pid мертвеца и растущий аптайм:
		// потребитель статуса выводит running из PID снимка и о смерти узнать
		// неоткуда (стенд 2026-08-28 — тумблер инстанса переставал работать,
		// потому что UI считал остановленный инстанс запущенным).
		l.forgetSnapshot()
...
// forgetSnapshot стирает состояние умершего воплощения: следующий Snapshot
// честно отвечает «процесс ещё ничего не рассказывал».
func (l *Link) forgetSnapshot() {
	l.mu.Lock()
	l.last = nil
	l.mu.Unlock()
}
