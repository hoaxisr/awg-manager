...
//
// ГРАНИЦА ГАРАНТИИ (I-3). «Ошибка List — отказ, ни одной записи не тронуто»
// верно только для отказа уровня КАТАЛОГА. Пофайловые беды List проглатывает
// сам: ошибку чтения одного файла он пропускает через continue без ошибки
// (awg_store.go:55-60), а битый JSON молча переименовывает в <id>.json.corrupt
// (:63-68). Отсюда два следствия, которых в слове «перечислить» не видно:
// наша нечитаемая запись выпадает из списка ТИХО — уборка её не подберёт, а
// гейт посева сочтёт, что терять нечего; и сам вызов не только перечисляет,
// но и попутно карантинит — хотя MarkSeeded зовёт его всего лишь чтобы
// посчитать. Лечится не здесь: границу двигать значит править internal/storage.
