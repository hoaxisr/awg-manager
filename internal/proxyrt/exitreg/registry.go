...
// DropMirror снимает зеркальную запись выхода, которого больше нет, —
// адресно и МИМО гейта посева. Звателей два: удаление инстанса и смена
// режима raw→wg, где выход исчезает из ведомости у ЖИВОГО инстанса.
//
...
// процесса, показывая пользователю карточку туннеля без выхода.
//
...
func (r *Registry) DropMirror(id, ownerInstanceID string) error {
	...
	if removed {
		r.log.Info("exit-mirror-removed", id, "зеркальная запись удалена адресно: выхода больше нет")
	}
	return nil
}
