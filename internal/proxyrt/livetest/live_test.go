...
func TestLiveEvictionIsTerminalForOldManager(t *testing.T) {
	...
	// Ждём, пока старое соединение доразберёт поток. Конец чтения приходит из
	// него самого и означает, что кадр evicted УЖЕ разобран: порядок в разборе
	// жёсткий — защёлка, буферизация, следом ошибка чтения. Пока наблюдатель
	// задержан, State ходит по тому же мёртвому соединению и переподключений
	// не делает.
	//
	// Конец чтения — EOF ИЛИ ECONNRESET. Второй ядро отдаёт, когда процесс
	// закрыл сокет с непрочитанным кадром клиента в очереди: State из этого
	// цикла легко попадает в окно между accept второго и close в evict. Данные
	// до сброса ядро отдаёт всё равно, так что кадр evicted разобран и тут.
	deadline := time.After(5 * time.Second)
	for {
		_, err := first.State(context.Background())
