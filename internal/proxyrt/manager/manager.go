...
func (m *Manager) Update(ctx context.Context, key string, mutate func(*instancestore.Record) error) error {
	...
	if mg, ok := m.m[key]; ok {
		...
		// конфига всех четырёх ролей, импорт ссылки, обновление подписки и
		// смена намерения. Любая из них — ручное действие, которое могло
		// устранить причину отказа старта, поэтому пауза анти-флаппинга
		// снимается здесь, а не швами по продуктовым пакетам: их пришлось бы
		// заводить пятью копиями и одну всё равно забыть. Цена принята: сброс
		// шире прежнего — старый мир на смене намерения его не делал.
		//
