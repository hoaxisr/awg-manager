...
type fileFormat struct {
	...
	// Пишут ТРИ пути: посев, боот и правка инстанса (`Manager.update`) —
	// первоисточник этого поля, поэтому здесь перечень полный. Лежат на диске
...
type State struct {
	...
	// за порт (амендмент G2 — посев; позже к нему добавились боот и правка).
