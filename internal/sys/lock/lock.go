...
// New creates a new lock with the given name.
func New(name string) *Lock {
	return NewWithDir(name, LockDir)
}
...
// WaitLock attempts to acquire the lock with retries until timeout.
func WaitLock(name string, timeout time.Duration) (*Lock, error) {
	return WaitLockDir(name, LockDir, timeout)
}
