...
func NewExternalTunnelsHandler(svc ExternalTunnelService, tunnelSvc TunnelService, store *storage.AWGTunnelStore) *ExternalTunnelsHandler {
	return &ExternalTunnelsHandler{svc: svc, tunnelSvc: tunnelSvc, store: store}
}

// SetLoggingService sets the logging service for the handler.
func (h *ExternalTunnelsHandler) SetLoggingService(logger LoggingService) {
	h.logger = logger
}
...
func (h *ExternalTunnelsHandler) Adopt(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if h.logger != nil {
			h.logger.LogError(logging.CategoryTunnel, "adopt", interfaceName, "Failed to adopt external tunnel", err.Error())
		}
	...
	if h.logger != nil {
		h.logger.Log(logging.CategoryTunnel, "adopt", result.Name, "External tunnel adopted")
	}
