...
func NewPingCheckHandler(service PingCheckService, tunnels *storage.AWGTunnelStore, nwgOp *nwg.OperatorNativeWG) *PingCheckHandler {
...
func (h *PingCheckHandler) ConfigureTunnelPingCheck(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategoryTunnel, "ping-check-configure", id, "Ping-check configured: host="+cfg.Host)
	}
...
func (h *PingCheckHandler) RemoveTunnelPingCheck(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategoryTunnel, "ping-check-remove", id, "Ping-check removed")
	}
