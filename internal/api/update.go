...
func NewUpdateHandler(updater *updater.Service) *UpdateHandler {
	return &UpdateHandler{updater: updater}
}

// SetLoggingService sets the logging service.
func (h *UpdateHandler) SetLoggingService(logger LoggingService) {
	h.logger = logger
}
...
func (h *UpdateHandler) Apply(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategorySystem, "update", "", "Starting update from GitHub release")
	}
