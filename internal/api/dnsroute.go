...
func NewDNSRouteHandler(svc DNSRouteService) *DNSRouteHandler {
	return &DNSRouteHandler{svc: svc}
}

// SetLoggingService sets the logging service for the handler.
func (h *DNSRouteHandler) SetLoggingService(logger LoggingService) {
	h.logger = logger
}
...
func (h *DNSRouteHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategoryDnsRoute, "dns-route-create", created.ID,
			"DNS route list created: "+created.Name)
	}
...
func (h *DNSRouteHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategoryDnsRoute, "dns-route-update", updated.ID,
			"DNS route list updated: "+updated.Name)
	}
...
func (h *DNSRouteHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategoryDnsRoute, "dns-route-delete", id, "DNS route list deleted")
	}
...
func (h *DNSRouteHandler) SetEnabled(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		...
		h.logger.Log(logging.CategoryDnsRoute, "dns-route-toggle", id,
			"DNS route list "+action)
	}
...
func (h *DNSRouteHandler) Refresh(w http.ResponseWriter, r *http.Request) {
	...
	if id != "" {
		...
		if h.logger != nil {
			h.logger.Log(logging.CategoryDnsRoute, "dns-route-refresh", id,
				"DNS route subscriptions refreshed")
		}
	} else {
		...
		if h.logger != nil {
			h.logger.Log(logging.CategoryDnsRoute, "dns-route-refresh-all", "",
				"All DNS route subscriptions refreshed")
		}
	}
