...
func (m *mockPingCheckService) ResumeMonitoring(tunnelID string) {}

func (m *mockPingCheckService) ResetFailCount(tunnelID string) {}
...
func TestPingCheckHandler_GetStatus(t *testing.T) {
	...
	handler := NewPingCheckHandler(svc)
...
func TestPingCheckHandler_CheckNow_Disabled(t *testing.T) {
	...
	if rec.Code != http.StatusBadRequest {
		t.Errorf("Status code = %d, want %d (bad request when disabled)", rec.Code, http.StatusBadRequest)
	}
}
