...
type AmneziaPremiumHandler struct {
	...
	sessionKey string
	// keyGen — поколение состояния ключа: растёт ТОЛЬКО на удавшемся СБРОСЕ
	// состояния, то есть на удалении. SaveKey снимает поколение ДО похода в
	// портал и сверяет на возврате, потому что поход длится до таймаута
	// клиента: без сверки DELETE, пришедший в это окно, молча отменялся бы
	// вернувшимся SaveKey — тот безусловно вернул бы ключ и в память, и на
	// флеш. «Забудь мой секрет» обязано побеждать.
	//
...
// Зовётся из удаления — единственного места, где ключ перестаёт быть нашим:
// сессия добыта ИМ и жить дольше него не имеет права. Сброс тут грубый, на
// весь клиент, поэтому там, где сессия в кэше может быть уже не наша, его
// звать нельзя (см. отменённую ветку SaveKey). Клиент ради сброса не
// собирается — сбрасывать тогда нечего.
...
func (h *AmneziaPremiumHandler) SaveKey(w http.ResponseWriter, r *http.Request) {
	...
	if cancelled {
		...
		// на диске.
		//
		// Сессию портала при этом НЕ роняем. Ронять её можно только целиком, а
		// в кэше клиента к этому моменту может лежать ЧУЖАЯ — заведённая
		// сохранением, которое прошло, пока мы висели в портале: сбросить её
		// значит выгнать работающую подписку. Своя же сессия привязана к
		// отпечатку ключа (amneziacp.Client.session), и запрос другим ключом
		// промахнётся мимо неё сам, а забранным ключом запросов больше не
		// будет. Прицельного сброса (ср. amneziacp.dropSession — «роняем,
		// только если сессия всё ещё та») здесь нет: CheckKey не отдаёт наружу
		// идентификатор сессии, которую добыл.
...
func (h *AmneziaPremiumHandler) DeleteKey(w http.ResponseWriter, r *http.Request) {
	...
	// Забвение памяти, стирание шифротекста и сдвиг поколения идут ПОД ОДНИМ
	// захватом — тем же, под которым пишет commitKey. Вынеси стирание наружу —
	// и летящий SaveKey встраивается между сдвигом и стиранием: поколение он
	// снимет уже сдвинутым, гейт его пропустит, он запишет ключ и ответит
	// stored=true, а хвост удаления сотрёт записанное. Ключ остался бы в
	// памяти демона и пропал с диска, то есть подписка терялась бы при
	// перезапуске.
	//
	// Память забывается ДО записи: если запись не удастся, у нас останется
	// меньше секрета, а не больше.
	//
	// Поколение двигается ТОЛЬКО после удавшегося стирания: сдвиг на неудаче
	// отменял бы летящее законное сохранение (409 «введите ключ заново»), не
	// удалив при этом ничего.
	h.mu.Lock()
	h.sessionKey = ""
