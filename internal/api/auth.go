...
func (h *AuthHandler) SetLoggingService(logger LoggingService) {
	h.logger = logger
}

// NewAuthHandler creates a new auth handler.
func NewAuthHandler(keenetic *auth.KeeneticClient, sessions *auth.SessionStore, settings *storage.SettingsStore) *AuthHandler {
	return &AuthHandler{
		keenetic: keenetic,
		sessions: sessions,
		settings: settings,
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.keenetic.Authenticate(r.Context(), req.Login, req.Password); err != nil {
		if errors.Is(err, auth.ErrInvalidCredentials) {
			if h.logger != nil {
				h.logger.LogWarn(logging.CategorySystem, "login", req.Login, "Login failed: invalid credentials")
			}
			response.ErrorWithStatus(w, http.StatusUnauthorized, "Неверный логин или пароль", "AUTH_FAILED")
		} else {
			if h.logger != nil {
				h.logger.LogError(logging.CategorySystem, "login", req.Login, "Login failed: router unavailable", err.Error())
			}
	...
	if h.logger != nil {
		h.logger.Log(logging.CategorySystem, "login", req.Login, "User logged in")
	}
...
func (h *AuthHandler) Logout(w http.ResponseWriter, r *http.Request) {
	...
	if h.logger != nil {
		h.logger.Log(logging.CategorySystem, "logout", "", "User logged out")
	}
