...
const backupUploadMaxBytes = 512 << 20
...
func (h *BackupHandler) Export(w http.ResponseWriter, r *http.Request) {
	...
	if err := backup.Export(h.dataDir, h.appVersion, &buf); err != nil {
...
func (h *BackupHandler) Import(w http.ResponseWriter, r *http.Request) {
	...
	limited := io.LimitReader(file, backupUploadMaxBytes+1)
	quiesced := false
	if h.quiesce != nil {
	...
	if err := backup.Restore(h.dataDir, limited); err != nil {
		if quiesced && h.resume != nil {
			h.resume(r.Context())
		}
		if h.log != nil {
