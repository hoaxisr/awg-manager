...
func (h *SettingsHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	// Validate bootDelaySeconds
	if settings.BootDelaySeconds != 0 && (settings.BootDelaySeconds < 120 || settings.BootDelaySeconds > 600) {
		response.ErrorWithStatus(w, http.StatusBadRequest, "Invalid bootDelaySeconds: must be between 120 and 600", "INVALID_BOOT_DELAY")
		return
	}

	// Validate backendMode
	if settings.BackendMode != "" && settings.BackendMode != "auto" && settings.BackendMode != "kernel" && settings.BackendMode != "userspace" {
		response.ErrorWithStatus(w, http.StatusBadRequest, "Invalid backendMode: must be auto, kernel, or userspace", "INVALID_BACKEND_MODE")
		return
	}
	...
	if h.logger != nil {
		...
		if oldSettings.BackendMode != settings.BackendMode {
			h.logger.Log(logging.CategorySettings, "backend", "", "Backend mode changed to "+settings.BackendMode+" (restart required)")
		}
		if oldSettings.BootDelaySeconds != settings.BootDelaySeconds {
			h.logger.Log(logging.CategorySettings, "boot-delay", "", fmt.Sprintf("Boot delay changed to %ds", settings.BootDelaySeconds))
		}
