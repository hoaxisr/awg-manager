...
- **Ключ — единственная защита эндпоинта.** Ограничение попыток считает их по IP, а через реверс-прокси KeenDNS все удалённые клиенты приходят с одного адреса самого прокси, так что от перебора снаружи оно не спасает. Защищает длина ключа (256 бит), поэтому ключ хранят как пароль и отзывают сразу, как только он стал не нужен.
