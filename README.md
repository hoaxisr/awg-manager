...
Доступ только по ключу (даже если авторизация веб-интерфейса выключена). Доступны просмотр состояния, логов, туннелей и маршрутов и безопасные изменения (старт/стоп туннелей, DNS/статические/клиентские маршруты, sing-box). Удаление туннелей, бэкапы, обновления и файлы через MCP недоступны.
